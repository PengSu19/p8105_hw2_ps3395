...
Three datasets are involved in Problem 2, including *"Mr."*, *"Professor"* and *"Gwynnda"* Trash Wheel. Which containing `r nrow(Mr_trash)`, `r nrow(professor_trash)`, and `r nrow(gwynnda_trash)` observations respectively. Moreover, there are common variables involved in these three datasets, such as,
...
In addition, as the study was focused on the MCI, participants who did not have MCI at the baseline do not meet the stated inclusion criteria and were removed by `filter` function. After all cleaning steps, only `r nrow(mci_baseline)` observations remained with `r ncol(mci_baseline)` variables. Then filter the participants that did not develop MCI at the end, and finally only `r nrow(mci_baseline_finall)` participants \newline

In conclusion, `r nrow(mci_baseline_na)` participants were recruited, and only `r nrow(mci_baseline_finall)` of these developed MCI at the end. Furthermore, the average baseline age is `r round(mean(pull(mci_baseline,current_age)),2)` and there are `r round(nrow(filter(mci_baseline,sex == "female", apoe4 == "carrier"))/nrow(filter(mci_baseline,sex == "female")) *100,2)`% of women in the study carried APOE4.
...
### Comment 
Comparing cleaned baseline and amyloid datasets, there are `r length(unique(pull(participants_only_in_baseline,id)))` participants appearing only in the baseline data, and `r length(unique(pull(participants_only_in_amyloid,id)))` participants only in amyloid data.
...
### Description 
After combining baseline and amyloid data with participants that appearing in both datasets, which is `r length(unique(pull(merge_mci,id)))`, the merging dataset *"merge_mci"* finally includes `r nrow(merge_mci)` observations with `r ncol(merge_mci)` variables that from two datasets.